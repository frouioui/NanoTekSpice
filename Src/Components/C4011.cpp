...
C4011::C4011() :
Component::MyComponent(nts::C4011)
{
...
nts::Tristate C4011::computeInput(nts::Pin &pin)
{
...
nts::Tristate C4011::computeOutput(nts::Door &door)
{
...
void C4011::checkSelfLink(std::size_t pin1, std::size_t pin2)
{
    bool input = false;
    bool output = false;
    ...
    if (!(input == true && output == true))
        throw Error::Parser::FileError("No corresponding pin", "C4011::checkSelfLink");
}
...
void C4011::setLink(std::size_t pin , nts::IComponent &other, std::size_t otherPin)
{
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4011::setLink");
}

void C4011::setInput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    for (auto it = _doors.begin(); it != _doors.end() && find == false; ++it) {
        if (it->second.input1.pin == pin) {
            if (it->second.input1.destinationName != nullptr)
                throw Error::Component::LinkError("Pin already linked", "C4011::setInput");
            it ->second.input1.destinationName = &other;
            it->second.input1.destinationPin = otherPin;
            find = true;
        }
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4011::setInput");
}

void C4011::setOutput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4011::setOutput");
}
