...
C4081::C4081() :
Component::MyComponent(nts::C4081)
{
...
nts::Tristate C4081::computeInput(nts::Pin &pin)
{
...
nts::Tristate C4081::computeOutput(nts::Door &door)
{
...
void C4081::checkSelfLink(std::size_t pin1, std::size_t pin2)
{
    bool input = false;
    bool output = false;
    ...
    for (auto it = _doors.begin(); it != _doors.end(); ++it) {
        ...
        ((it->second.output.pin == pin1 || it->second.output.pin == pin2)))
            throw Error::Component::LinkError ("Door linked to itself", "C4081::checkSelfLink");
    ...
    if (!(input == true && output == true))
        throw Error::Parser::FileError("No corresponding pin", "C4081::checkSelfLink");
}
...
void C4081::setLink(std::size_t pin , nts::IComponent &other, std::size_t otherPin)
{
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4081::setLink");
}

void C4081::setInput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    for (auto it = _doors.begin(); it != _doors.end() && find == false; ++it) {
        if (it->second.input1.pin == pin) {
            if (it->second.input1.destinationName != nullptr)
                throw Error::Component::LinkError("Pin already linked", "C4081::setInput");
            it ->second.input1.destinationName = &other;
            it->second.input1.destinationPin = otherPin;
            find = true;
        }
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4081::setInput");
}

void C4081::setOutput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4081::setOutput");
}
