...
C4030::C4030() :
Component::MyComponent(nts::C4030)
{
...
nts::Tristate C4030::computeInput(nts::Pin &pin)
{
...
void C4030::checkSelfLink(std::size_t pin1, std::size_t pin2)
{
    bool input = false;
    bool output = false;
    ...
    for (auto it = _doors.begin(); it != _doors.end(); ++it) {
        ...
        ((it->second.output.pin == pin1 || it->second.output.pin == pin2)))
            throw Error::Component::LinkError ("Door linked to itself", "C4030::checkSelfLink");
    ...
    if (!(input == true && output == true))
        throw Error::Parser::FileError("No corresponding pin", "C4030::checkSelfLink");
}
...
void C4030::setLink(std::size_t pin , nts::IComponent &other, std::size_t otherPin)
{
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4030::setLink");
}

void C4030::setInput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    for (auto it = _doors.begin(); it != _doors.end() && find == false; ++it) {
        if (it->second.input1.pin == pin) {
            if (it->second.input1.destinationName != nullptr)
                throw Error::Component::LinkError("Pin already linked", "C4030::setInput");
            it ->second.input1.destinationName = &other;
            it->second.input1.destinationPin = otherPin;
            find = true;
        }
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4030::setInput");
}

void C4030::setOutput(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
    bool find = false;
    ...
    if (find == false)
        throw Error::Parser::FileError("No corresponding pin", "C4030::setOutput");
}
